# 封臣协议设计（Vassal Protocol）— 基于 A2A 超集

> 状态：**现行（设计稿 v0.1，2026-09-21）**。实施进度记在 [handoff.md](../handoff.md)，本文只写设计。
> 决策：已拍板「直接复用 A2A 标准做超集，不自造最小协议」（deferred-items #1 已销项）。

## 0. 一句话定位

封臣协议 = **标准 A2A 协议 + 一层「封臣契约」扩展**：矩阵产品（agent-world、job-agent、agent-dev、pr-helper、atlas、loom 等）保持独立仓库与独立部署，通过标准 A2A 被 Zeus 发现与调用，再通过超集扩展满足「向 Zeus 负责」的战报、升级与治理要求。

**为什么是超集而不是子集**：
- Zeus 需要与任意外部 Agent 互操作（A2A 是立国三纲之一），自己先遵守标准才有资格要求别人。
- 超集意味着：**只懂标准 A2A 的调用方仍能调用封臣**（降级为基础 Agent），Zeus 的增强能力（战报、治理）是可选的增量，不是新的闭墙。
...
| 角色 | 说明 |
| --- | --- |
| **Zeus（宗主）** | 唯一面向用户的门面；发现、派遣、监督封臣与外部 Agent |
| **封臣（Vassal）** | 矩阵产品中以 headless 形态提供能力的 Agent，实现 A2A + 封臣扩展 |
| **外客（Guest Agent）** | 矩阵之外的外部 Agent，仅实现标准 A2A，无封臣义务 |
| **驾驶员（Driver）** | 人类用户/员工：派发、监督、中断、拍板的最高权限持有者 |

封臣与外客的唯一区别：封臣签署封臣契约（§4），外客不需要。
...
2. **任务模型**：支持 `tasks/send`（同步）与 `tasks/sendSubscribe`（SSE 流式）；封臣的每个能力映射为一个 skill + 一个任务语义。
...
### 4.1 Agent Card 扩展：fealty（封臣契约声明）
...
```json
{
  ...
  "x-zeus-fealty": {
    "version": "1",
    "swornTo": "zeus",              // 效忠对象，发布期由 bayjf 名册分配
...
封臣收到任务后必须在 `ackSeconds` 内转入 `working` 并回传首个状态事件，事件带扩展字段：
...
Zeus 注入 `x-zeus.runId` 实现全程追踪；封臣透传回传，不解释。

### 4.3 战报回流（Report-Back）

任务完成/失败时，artifact 之外附带一份**战报（report）**：
...
`escalationPolicy: auto` 的封臣在风险场景（不可逆操作、低置信度、越权请求）通过标准 A2A 的 `input-required` 状态 + 扩展字段升级：
...
Zeus 汇聚所有封臣的升级请求到**监督台**，按 Realm 与权限路由给正确的驾驶员。

### 4.5 治理（Governance）

- **审计**：fealty 声明 `dataPolicy: none` 的封臣，Zeus 不向其派发任何含 Realm 数据的任务；派发时脱敏。
- **背压**：封臣可在 status-update 中回传 `x-zeus: { "backpressure": "saturated" }`，Zeus 转为向其他封臣/队列分流。
- **权限回收**：Zeus 可随时吊销某封臣的 access token（标准 A2A 认证机制），吊销即降级为外客或拉黑，不需要封臣配合。
...
- 战报的成本字段单位与结算口径（跨封臣可比性）→ 触发条件：企业版计费立项时（联动 deferred #4）。
- 封臣饱和背压与 Zeus 任务队列的降级顺序 → 触发条件：≥3 个封臣在线后压测。
