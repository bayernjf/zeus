...
### #1 封臣协议形态
- **问题**：封臣协议直接复用 A2A 标准做超集，还是自定义最小协议？
- **影响**：决定矩阵产品（agent-world/job-agent/agent-dev/pr-helper/atlas/loom）的改造成本与外部 Agent 兼容性。
- **重启条件**：开始起草 design-vassal-protocol.md 前必须先定。
