...
> 状态：**现行 v0.31（2026-09-25，评审 v0.9 之后的首跑正确性批次：审计失败不再冒充派发失败、两扇门的 realm 语法统一、配置面可读）**；上一条 v0.30（藏宝图盖到内核状态文件 + 企业写凭证签名且一次性，deferred #13/#14 销项）；上一条 v0.29（E9.1/E9.2 上岗四道门与首日简报）；上一条 v0.28（企业域三级租户隔离 + 双域授权与审计呈现，E3.6/E6.4 收口）；v0.25（E4.8 阻塞归因纠偏，状态仍 🚧）；上一条 v0.24（2026-09-25，E1.5 有界并发队列 + E4.7 审计落盘可读 + E3.5 企业域收口 + 连接器/决策面接线 + 快照 0600）
...
| E6.4 | 双域共存授权界面 | P2 | ✅ | **已落地**（设计见 [design-realm](design-realm.md) §7.2–§7.5，deferred #6 销项）。**授权粒度**：`DomainGrant`（subject × realmId × access × grantedBy × expiresAt × nonce）只管**域边界**（个人域→企业域需显式授权；**企业域→个人域永远拒，连能表达它的凭证形状都不存在**），与 E3.5 的 `DriverWriteGrant` 分层不合并（前者"这主体能不能碰这域"、后者"这一次企业写被授权了"）；`decideRealmAccess()` 是**唯一**判定函数（派发取数与驾驶员探针共用，reason 精确到 `grant-expired`/`access-not-granted`/`no-grant`，因为这三者要求操作员做不同的事）；`DomainGrantRegistry` 令 **nonce 一次性**（吊销后仍算已用，防"撤销"变成重放入口）且随 KernelSnapshot 持久化（重启既不静默放宽也不静默丢失）。**取数与审计呈现**：新增 `resolveRealmSource()` —— 此前 `realmHits` 由调用方自报、内核从未自己进过任何 Realm，故"声明 personal 域却塞企业域内容"无从发现；现在内核按 realmId 自己取、核对**真实 type 与声明是否一致**（不符 `realm-type-mismatch` 拒）、过边界判定后才把命中连同 realmId 送进派发，**放行与拒绝都落同一根审计脊**（`domain-read`/`domain-refused`，reason 前缀）。**界面（无 UI 产品的运维面）**：`GET /api/domains`（挂载/租户级/只读/条目数/指纹，**绝不含绝对路径**）、`POST /api/domains/grants`、`DELETE /api/domains/grants/:id`、`GET /api/domains/access`（**干跑探针**，不读任何内容）、`GET /api/audit?decision=domain-refused`；`GET /api/state` 增 `enterpriseRealms`/`tenantScopedRealms`/`domainGrants`（"没标租户的企业域数"是操作员最该一眼看到的风险位）。测试 `tests/realm-authorization.test.ts`、`tests/realm-source.test.ts`、`tests/http-domains.test.ts`；植入缺陷验证：去掉企业域→个人域的拒绝分支 → 3 例红；关掉声明核对 → 2 例红 |
...
| ID | 需求 | 优先级 | 状态 | 验收标准 |
|---|---|---|---|---|
| E10.1 | CI（typecheck/test/build 矩阵） | P0 | ✅ | `.github/workflows/ci.yml`（Node 20/22 矩阵，npm ci → typecheck → test → build，零 secret）。**2026-09-25 首次真机确认**：dev 分支 push `206af94..3e91a9a` 触发 run 36024156638，两个矩阵各约 28s **全绿**（472/472，无 flaky 复现）——此前各批次只有"本地按 CI 序列实跑全绿"，GitHub 侧结果一直未观测。遗留注解（checkout@v4/setup-node@v4 target Node 20 被强制跑在 24、ubuntu-latest 2026-10-19 迁移）登记 deferred #12（action 版本部分已随 `@v7` 升级销项）。**同日复验三次全绿，CI 已是例行防线而非待办关口**：随后两批 push 分别由 run 36045209815（512/60 文件，30s/28s）与 run 36061395015（**532/61 文件**，26s/22s）在 Node 20.x / 22.x 双矩阵验过，无 flaky 复现；`ubuntu-latest` 2026-10-19 自动迁移那条 runner 层风险另登记 **deferred #16** |
| E10.2 | 真机部署：pr-helper + Zeus 门面 | P1 | ⬜ | 支撑 E4.8 真机验收与 Zeus↔loom 联调 |
| E10.3 | 库公共入口与构建产物 | P0 | ✅ | src/index.ts 聚合导出；dist 含 .d.ts |
| E10.4 | 并发压测与容量基线 | P1 | ✅ | **本机 mock 回环基线 v0.3**（[capacity-baseline.md](capacity-baseline.md)，`npm run bench:capacity`），五场景：A 扇出宽度、B 并发意图、**C H2 门面全链路（真实 `app.listen` 回环 TCP + bearer + JSON，128 分支点门面附加仅约 5–10ms 墙钟、吞吐约低 10–15%，低并发几乎无差）**、**D 高并发取消传播（128 个 input-required 挂起分支 14–18ms 全部取消、mock farm 实收 240 个 tasks/cancel 零丢失零重复）**；单意图扇出 ≤16 墙钟≈单封臣（内核附加 5–15ms）、128 在途分支零丢失、metrics 峰值准确、**E 并发闸门代价（v0.3，`--cap` 显式开启：128 分支负载下 cap 无限制/32/16/8 —— 实测 max in-flight 恰等于 cap、peak queue 恰等于 128−cap、128 分支零丢失；吞吐随 cap 近线性、尾延迟≈墙钟；绝对值跨次散布 ±2×，故只用比值与不变量下结论）**；附带把 `branchTimeoutMs` 从 20×think 提到 40×think 并注明它是防卡死看门狗而非 SLO（满载时对连续尾段误触发）。**口径限制**：D 取消的是已 settle 的非终态（input-required）分支，对仍 working、fanOut 未返回的进行中意图尚无按 intentId 的一等取消；真机容量（LLM/网络）待 ≥3 真实封臣 + deferred #9 有界队列后按同法重测 |
...
| M3 真机闭环 | E4.8、E10.2、Zeus↔loom 联调、E5.3 | 两封臣真机受调度；重启状态不丢。**2026-09-25 重估：本里程碑缺的不是工程而是一次执行**——pr-helper 已在生产（E4.8 只差对它跑一次验收脚本），Zeus 侧制品（Dockerfile/持久化/注册入口/H2 驾驶面）齐备，剩"起一个 Zeus 实例注册线上封臣 + 一次真机扇出" |
...
| v0.28 | 2026-09-25 | **E3.6 + E6.4 一批收口（企业域分级与双域授权）**：① `src/realm/tenant.ts` 三级 `TenantScope`（org/department/member，非 ASCII 段名可用、大小写折叠匹配、重连改租户即拒），**个人域不是租户**由 `connect` 抛错保证；层级是**结构性**边界（主体必须是 realm 租户的前缀或相等，可下探、不可上望旁视），**没有任何凭证能放宽它**。② `src/realm/authorization.ts` `DomainGrant` + `decideRealmAccess()` 单一判定（个人域→企业域须显式授权、企业域→个人域**永远拒**、reason 精确到 `grant-expired`/`access-not-granted`/`no-grant`）+ `DomainGrantRegistry`（**nonce 一次性**：吊销不等于复活，撤销后重放同 nonce 仍拒；随 KernelSnapshot 持久化，重启不静默放宽也不静默丢失）。③ **本轮真正的安全增量** `src/realm/source.ts`：此前 `realmHits` 全靠调用方自报、内核自己从没进过任何 Realm，"声明 personal 域却塞企业内容"无法发现；现在内核按 realmId 自取并**核对真实 type 与声明**，命中连同来源进派发，放行/拒绝都进同一根审计脊（`domain-read`/`domain-refused`）。④ 运维面 `GET /api/domains`（**不含绝对路径**）、`POST/DELETE /api/domains/grants`、`GET /api/domains/access` 干跑探针，`/api/state` 增 `enterpriseRealms`/`tenantScopedRealms`/`domainGrants`；env `ZEUS_REALM_ENTERPRISE="<root>::<tenant>"`（缺/坏 tenant **拒启**）。⑤ **三个守卫都做过缺陷植入**：恒真 `tenantReaches` → 5 例红（库/真实目录/HTTP 三层）；关掉声明核对 → 2 例红；删掉企业→个人拒绝分支 → 3 例红；另发现并删掉一处**看起来在防护实则永不生效**的长度比较（植入后 0 例红）。⑥ 顺手修：`docs/design-http-transport.md` 的"HTTP 不挂任何 Realm 路由"约束精确化为**内容 vs 治理元数据**（design-realm §6.4），否则新代码与已拍板决策互相打脸。deferred **#6 销项**，新登记 **#17**（改边界需显式操作：当前只能重启且会被漂移检查拒启）、**#18**（MCP 侧 actor 判定，触发条件 E3.4 真封臣）。新增 55 项测试，全量 **587 绿（66 文件）**，`tsc --noEmit`/build exit 0 |
| v0.29 | 2026-09-25 | **E9.1 + E9.2 一批收口（上岗门与首日简报）**：① 新组合层 `src/onboarding/`——**四道门现算不缓存**：seat（E9.3 名册）/ account（在册未吊销封臣）/ authorization（`decideRealmAccess`：E3.6 租户 + E6.4 授权）/ mentorship（E2.5 certified，**出勤不算能力**）；记录只存"谁开档、谁签字、谁豁免了什么"，资格每次重算，所以**签字之后吊销封臣或撤出名册会自动失去资格**，不依赖有人记得改状态位。② 三条严格化：`requiredSkills` 为空必须**显式豁免并写理由**（"没人说这岗位要会什么"是发现不是放行）、豁免不得覆盖已声明要求、**座位租户不得比其 realm 更宽**（`acme/eng` 的部门里放 `acme` 级座位等于让它看见兄弟部门）。③ `composeBriefing` 只从已有事实装配：职责/责任链/已认证与还缺技能/谁能教/组织惯例（本部门 realm 记忆召回，跨域读抛错）/边界两向；**`culture.totalFacts` 与命中数分开报**（"什么都没写"≠"写了但无关"），**`canWrite` 只表示域边界**、企业写入另需 `DriverWriteGrant`（`writeNeedsGrant` 单列），内容去时间戳后取**稳定 digest** 供任务证明"照哪一版简报派的活"。④ `first-task` 真走一次 `fanOut`（realm 取连接现状真实类型、目标限定该 Agent、params 带 commissionId+digest），未签字 409 `commission_gate`+`blockedOn`；审计脊新增 4 个 `commission-*` 决策；`/api/state` 增 `commissions`/`commissioned`；上岗记录进 KernelSnapshot。⑤ **真实进程冒烟揪出一个单测结构上看不见的洞**：fealty 缺 `dataRealms` 等字段的卡片能注册成功、派发时才以 `Cannot read properties of undefined (reading 'includes')` 崩——注册边界现按声明契约校验并点名缺哪个字段（空 `dataRealms` 合法＝"不承诺任何数据"，二极管会逐单给出理由）；三个测试 fixture 本身就是残缺卡（它们从没派过活所以一直没暴露），已补全。⑥ 门的可失效性三次植入验证（门常真 2 红 / 只信记录 3 红 / 能力门常过 2 红 / 校验关掉 4 红）。新增 33 项测试，全量 **620 绿（68 文件）**，`tsc --noEmit`/build exit 0；新文档 [design-onboarding](design-onboarding.md) v0.1 |
| v0.30 | 2026-09-25 | **deferred #13 + #14 一批收口（藏宝图盖到内核状态文件 / 企业写凭证签名且一次性）**：① **Vault v2**——`TreasureMap.source`（`realm`（含 E3.6 tenant）\| `files` 白名单）、`inventoryFromFiles`（缺失/绝对路径/`..` 逃逸/非普通文件/软链/二进制/超 64MiB 七类硬拒）、`LiveSource` 端口取代 L0 里硬绑的 `RealmStore`、CLI `--files-root/--files/--tenant` 且 `check\|restore` 不再要挂载参数；**顺带修掉两个真实缺陷**：带租户 scope 的企业 Realm 校验时丢 scope 重连被吞成 "root unreachable"（恢复工具对健康语料库喊狼来了），以及白名单包含性用未解析 root 比较 realpath 文件（macOS `/var→/private/var` 下**每个**白名单文件都被判逃逸）。② **E3.5 凭证**——`issueDriverWriteGrant`（内核铸 nonce + Ed25519 签 JCS，复用 `registry/signing.ts`）、`verifyDriverWriteGrant` 改 async 并按 形状→绑定→**验签**→有效期判定、`DriverGrantLedger` 落盘前消费 nonce 且随内核快照持久化、`FsRealmStoreOptions.now` 注入时钟（消灭 `expiresAt: 2099-…` 那种伪装的"仍然有效"）；`POST /api/realm/write-grants`、日记缺授权返回 **403**、审计脊 `driver-grant-issued`/`realm-write`、`GET /api/state` 报 `driverGrants.authority`（signed / shape-only 可见）。③ **验证**：`kernel.json` 毁库演练走真 CLI（备份→校验→删除→退出码 2→恢复→字节级 sha256 相同→用恢复出的文件 boot 起内核），签名/防重放/持久化三个守卫各做缺陷植入（去验签 2 红、账本永不拒 9 红、消费不落盘 1 红），编译产物真进程冒烟 13 项全过（含重启后仍拒重放）。全量 **658 绿（70 文件）**，typecheck/build 过 |
| v0.31 | 2026-09-25 | **评审 v0.9 之后的首跑正确性批次（E4.7 审计面 + 部署配置面；全量 665 绿 / 70 文件，tsc/build 过）**：① **审计失败不再冒充派发失败**——按 `.env.example` 默认值起服务时，每条意图分支的 `reason` 实测是 `ENOENT … audit.jsonl`（sink 假定目录存在，异常逃出 dispatcher 分支 `catch` 后被重新标成"封臣失败"；A/B 对照定位并复验修复）。现在构造期建目录与文件（审计路径不可用 → **拒启并点名 `ZEUS_AUDIT_FILE`**），运行期写失败只计数不抛（首次告警、后续静默计数），`GET /api/state` 增 `audit.failures/degraded`；写出固定 **0600**，且**轮转后重新收紧**（轮转正是把固定 mode 悄悄退回 0644 的那道缝）。② `ZEUS_REALM_ROOTS` 两扇门统一为逗号（MCP stdio 此前按 `[:;]` 切，冒号会切碎 Windows 盘符路径）——实测同一目录在 HTTP 内核与 stdio 进程里给出**同一个 realmId**。③ `.env` 由谁解析写进文件头（全仓库无 dotenv：docker `--env-file` 与 systemd 会读，裸机 `npm start` 不读，并给出 `node --env-file=` 这条真能用的路）。④ 启动日志逐条打印挂载了哪些 realm（id/类型/租户/可写性），`ZEUS_VASSAL_SEEDS` 拉不到卡片改为一行可读拒启。⑤ `deployment.md` §2 补上缺的 **10 行** env（代码读 24 个、表内原 14 行）。**6 次缺陷植入全部被接住**（4/1/1/1/1/1 例红；其中一次植入把源码改成语法错误，判为废植入、换合法等价植入重跑），真进程复跑三条。**E4.7 状态仍 ✅，改的是它能否在默认配置下工作；MVP 判定仍 ❌**（评审 §E 第 2–4 条未动：封臣出站凭证、Skill 上派发路径、MCP tools+actor）。另记一条**待拍板**不一致：`ZEUS_JUDGE_THRESHOLD` 非数字被静默忽略（`tests/boot-decision.test.ts:70-76` 把它钉成契约），与 E1.5"非法值拒启"相反 |
