...
> 状态：**现行 v0.29（2026-09-25，E9.1/E9.2 上岗四道门与首日简报落地，P0 仅剩 E4.8 真机验收）**；上一条 v0.28（企业域三级租户隔离 + 双域授权与审计呈现，E3.6/E6.4 收口）；v0.25（E4.8 阻塞归因纠偏，状态仍 🚧）；上一条 v0.24（2026-09-25，E1.5 有界并发队列 + E4.7 审计落盘可读 + E3.5 企业域收口 + 连接器/决策面接线 + 快照 0600）
...
| E3.5 | write 与驾驶员授权凭证 | P1 | ✅ | **已收口**（`FsRealmStore.write` + `src/realm/grant.ts`：personal 默认可写、readOnly 连接拒写（即使持凭证）、enterprise 写须 `verifyDriverWriteGrant` 校验形状/绑定域/有效期；tmp+rename 原子写、延续路径穿越/symlink/文本扩展名/1MiB 防护、写后 connect 快照与 `contentDigest`/itemCount 一致、审计回调记 `grantedBy`）。**2026-09-25 补上关键一环**：`connect` 不再拒 enterprise，且存储类型如实记录（此前 `realms.set` 硬编码 `personal`，企业域写闸门在真实 store 上永远走不到、只在纯函数测试里被 mock 打桩＝死代码）；企业域 connect→授权写→读回→审计全链路有测试（26 项 realm-write）。**仍属 P1 的半边**：MCP `tools/write` 暴露与凭证签发/签名，随 E3.4 read-realm 封臣触发；enterprise 多租户分级是 E3.6，另一条需求 |
