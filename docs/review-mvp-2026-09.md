...
> 状态：**现行（评审报告 v0.12，2026-09-25，对 v0.10/v0.11 的 ✅ 做独立复核：7/8 条由本评审在真进程/真 socket/真容器上重跑复现，判定维持 ✅；新抓一条"文档承诺 502、实现返回 400"的错误码缺陷，并补登记 #19 入站 A2A 面 / #20 参数校验口径不一致）**。评审对象：Zeus 仓库 `dev` 分支 HEAD `8ddcddc`（v0.9 起点 HEAD `f4727f1`；v0.5/v0.6 为纯文档更正与销项，v0.7/v0.8/v0.9/v0.10 含代码；**v0.9 把"操作者能不能摸到三条接入通道"当问题重问，v0.10 把三条接入通道缺的库内执行点全部写成代码，v0.11 把"对线上跑一次验收"与 docker 实构实跑两条仓库外执行动作实际跑掉，v0.12 不采信上述记录、把能本机复现的判定全部亲手重跑——判定以 v0.12 为准，其下各节为历史**）。
...
| v0.14 | 2026-09-26 | **验证面本身变了（配置层，代码与用例零改动）**：deferred **#15** 销项，选 B——`engines.node: ">=22.0.0"` + `.npmrc` 的 `engine-strict=true` + `.nvmrc` 22 + CI 矩阵 `[20.x, 22.x]` → **`[22.x, 24.x]`**。因此**此后所有评审证据的运行时口径要改写**：v0.6 起的"Node 20.x / 22.x 双矩阵全绿"现在是 **22.x / 24.x**（v0.13 起 runner 亦钉 `ubuntu-24.04`）。证据强度未变（三档 704 绿与 22/24 真进程冒烟出自我上一批实测，本批未重跑），**变的是"我们声明支持什么"**——Node 20 自 2026-03-24 起零发布，不再被声明支持。MVP 判定不变；新增一条待观察项：**22/24 双矩阵的云端首绿尚未发生**（本批未 push） |
