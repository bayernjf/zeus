...
> 状态：**现行（评审报告 v0.12，2026-09-25，对 v0.10/v0.11 的 ✅ 做独立复核：7/8 条由本评审在真进程/真 socket/真容器上重跑复现，判定维持 ✅；新抓一条"文档承诺 502、实现返回 400"的错误码缺陷，并补登记 #19 入站 A2A 面 / #20 参数校验口径不一致）**。评审对象：Zeus 仓库 `dev` 分支 HEAD `8ddcddc`（v0.9 起点 HEAD `f4727f1`；v0.5/v0.6 为纯文档更正与销项，v0.7/v0.8/v0.9/v0.10 含代码；**v0.9 把"操作者能不能摸到三纲"当问题重问，v0.10 把三纲缺的库内执行点全部写成代码，v0.11 把"对线上跑一次验收"与 docker 实构实跑两条仓库外执行动作实际跑掉，v0.12 不采信上述记录、把能本机复现的判定全部亲手重跑——判定以 v0.12 为准，其下各节为历史**）。
...
|---|---|---|
| 出站凭证：要 bearer 的真封臣派发得出去 | **实测**（`dist/http/serve.js` 真进程 + 本地真 socket 回环封臣） | 封臣侧日志实收 `Authorization: Bearer sekrit-8f2c` ✅ |
| token 绝不回显 | **实测** | `GET /api/roster`、`/api/roster/public`、`/api/state` 三处 grep 该 token **0 命中** ✅ |
| 吊销即刻断流 | **实测** | 吊销后再派发，封臣侧 auth 行数 `2 -> 2`（零新增） ✅ |
| 重启后凭证仍在 | **实测** | **去掉 seed 环境变量**重启、仅靠状态文件恢复：vassals=1 且再次带上同一 token ✅ |
| Skill 上派发路径（卸载真夺权） | **实测** | 三态成立：未注册 → pass-through；注册但无 active 版本 → 不派发并审计 `refused-skill-uninstalled`；显式 `request.vassals` 旁路**按设计保留**（驾驶员故意覆盖） |
...
两处文档写着 `POST /api/vassals` **"卡片不可达 502"**：本文 `:16`（v0.3 历史块）与 `handoff.md:130`。（本评审初稿一度写成"三处、含 PRD `E5.5`"，复核后剔除：`grep -n 502 docs/prd.md` 只有两条，都是连接器的握手/上游失败，与封臣卡片无关。）真进程实测只对了一半：
...
- **同一份文件里已有正确写法可抄**：连接器调用面（`src/http/server.ts:1317-1319`）是"按类型映射，未识别的抛出默认 502"，即**默认落在网关码上、把 400 留给真的是请求体错的情形**。封臣注册这条路由用的是消息正则，方向反了。
...
- **产品核心完全可用的 MVP：✅ 维持**。本轮把依据从"另一会话的记录"换成"本评审亲手复跑"：上表 8 条里 7 条实测复现，覆盖三纲三条支柱 + 部署形态 + 持久化与重启恢复。
- **不随之升格的部分（如实标注）**：① E4.8 对线上那一次仍是**记录**，不是本次证据；② §B 错误码缺陷未修，属操作性/可诊断性问题，不影响核心可用性；③ 仓库外仍差 Zeus↔loom 真机联调、Jev endpoint/key 核对、RSK 托管与公钥发布（#7）、#18 MCP actor 判定。
- **本轮新登记两条结构性缺口**（登记而非半做，见 deferred-items）：**#19 入站 A2A 面**——外部 Agent 调不进 Zeus（`src/http` 无 `/.well-known/agent-card.json`、无任何 `tasks/*` 路由）；v0.9 §A 已判定过，但从未进 deferred 清单，属于"评审说过、没人接"的失物。**#20 `ZEUS_JUDGE_THRESHOLD` 与 boot 参数校验口径不一致**——非数字被静默忽略（`src/state/boot.ts:537-540`）而并发类参数非法即拒启，`deployment.md:35` 已标注，统一它=推翻 `tests/boot-decision.test.ts:70-76` 钉住的契约，等拍板。
