...
| S14 | 多模型异构调度 | 按子任务难度路由强/便宜/快/本地模型；**Jev 模型落此层（见 §D）** | 📝 |
...
- **Jev 模型（2026-09-22 负责人提及）**：性质/提供方/能力未知；联网搜索通道 403 未能核实。已在 [handoff.md](../handoff.md)「New inputs / 待确认」登记。**拿到介绍或一句话定位后**：初判属于 S14（多模型异构调度）的候选模型，再决定是否进 PRD；事实未明前不做设计假设。
