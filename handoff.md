...
39. **第二批次：能搞的都搞（2026-09-25 ✅ 完成，库内闭环 + 工程卫生，全量 512 绿 / 60 文件，tsc/build 过，PRD v0.24）**：
   > 上一批收尾时我列了 A–G 七项候选，用户裁决"能搞的话都搞"。落地 7 个 commit；C（内核状态文件进藏宝图）与 F（S3 DAG 驾驶员入口）经实测**不是接线而是设计变更**，登记 deferred 而不是半做。
   ...
   - **验证**：`tsc --noEmit` exit 0、`npm run build` exit 0、`npx vitest run` **60 文件 512 测试全绿**（472→512：connectors 9 + 审计读端 6 + http-audit 6 + boot 审计与 concurrency 5 + semaphore 5 + backpressure 6 + realm enterprise 5，其余为改造既有用例）。**本批 7 commit 未 push（等授权）**；提醒：本机 shell 是 Node **20.20.2**，与 CI 的 20.x 矩阵一致。
...
* [docs/review-mvp-2026-09.md](docs/review-mvp-2026-09.md) — 项目级评审 **现行 v0.5（2026-09-25 更正；v0.4 基线 436 绿）**：功能性/完整度/可上线三维度、四场景容量证据、v0.1 五硬阻塞现状重判（代码/制品侧均已有对应物）、**MVP 重判：M1/M2 达成、M3 制品就绪真机未验，库内已无 P0 功能缺口，产品级上线仅剩仓库外真机/凭证/发布动作**；**v0.5 只更正一处归因：E4.8 的卡点不是"待 pr-helper 部署"（它早已上线），而是从没人对线上跑过一次验收，M3 因此是"差执行不差工程"**；原 v0.1–v0.4 快照保留 ★
* [docs/prd.md](docs/prd.md) — 产品需求文档 现行 v0.25（2026-09-25）：9 个 Epic、需求拆解（优先级/状态/验收标准）、里程碑与成功指标；v0.22 落签名链 v1.1 internal 名册封签（E5.2，含 revoked 两态 attestation）、容量四场景（E10.4 v0.2）、评审 v0.4，v0.21 接通 org/diary 持久化与驾驶员 HTTP、v0.20 落 E8.3 Diary/E9.3 Org、v0.19 落 E3.5 Realm write（grant 凭证）、v0.18 落 E1.3 adversarial judge、v0.16 落 E3.7 Vault CLI、v0.15 落 Vault 藏宝图（E8.1/E8.2 ✅）、v0.14 索引架构立场 design-agentic-integration ★
